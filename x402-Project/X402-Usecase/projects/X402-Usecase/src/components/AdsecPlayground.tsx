      ...
      {auditResponse && (
        <div className="space-y-6">
          ...
          {auditResponse.attestation && (
            <div className="bg-slate-900/70 border border-emerald-500/40 rounded-2xl p-5 shadow-lg">
              <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
                <h4 className="font-bold text-emerald-400 flex items-center gap-2">
                  <span className="font-mono">[✓]</span> Cryptographic On-Chain Attestation
                </h4>
                <span className="text-xs bg-emerald-500/10 text-emerald-300 font-mono px-3 py-1 rounded-full border border-emerald-500/40">
                  {auditResponse.attestation.status}
                </span>
              </div>
              <div className="space-y-1.5 text-xs font-mono text-slate-300 bg-[#05070d] p-3 rounded-lg border border-slate-800 thin-scroll overflow-x-auto">
                <div><span className="text-slate-600">code sha256     :</span> {auditResponse.attestation.codeHash}</div>
                <div><span className="text-slate-600">tx note schema  :</span> {auditResponse.attestation.txNoteSchema}</div>
                <div><span className="text-slate-600">authority       :</span> {auditResponse.attestation.attestationAuthority}</div>
              </div>
            </div>
          )}
          ...
          {auditResponse.receipt && (
            <div className="bg-[#05070d] border border-slate-800 rounded-xl p-4 text-xs font-mono text-slate-500 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
              <div>
                network: <span className="text-slate-300">{auditResponse.receipt.network || 'Algorand TestNet'}</span>
                {auditResponse.receipt.paidAmount && <> · paid: <span className="text-amber-300">{auditResponse.receipt.paidAmount}</span></>}
              </div>
