...
export type TerminalPhase = 'idle' | 'recon' | 'challenge' | 'signing' | 'settling' | 'patching' | 'success' | 'error'
...
function attackFrames(): string[] {
  ...
  frames.push(['', '[✓] Static analysis & rule evaluation complete', '', ...labels.map((l) => `[${l}] ${'#'.repeat(W)}||`), ''].join('\n'))
  return frames
}
...
const THEMES: Record<TerminalPhase, PhaseTheme> = {
  idle: { label: 'STANDBY', step: 'Ready for request', text: 'text-base-400', chip: 'bg-base-800 text-base-300 border-base-700' },
  recon: {
  ...
  signing: {
    ...
    text: 'text-base-200',
    chip: 'bg-base-700 text-base-100 border-base-600',
  },
  settling: {
  ...
  error: { label: 'INTERRUPTED', step: 'Request halted', text: 'text-red-400', chip: 'bg-red-500/10 text-red-300 border-red-500/30' },
}
...
function framesFor(phase: TerminalPhase): string[] {
  switch (phase) {
    ...
    case 'recon':
      return RECON_FRAMES
    case 'challenge':
      return CHALLENGE_FRAMES
    case 'signing':
      return SIGNING_FRAMES
    case 'settling':
...
export const AsciiTerminal: React.FC<AsciiTerminalProps> = ({
  ...
  return (
    <div className={`rounded-xl border border-base-800 bg-base-ink shadow-pop overflow-hidden font-mono ${glow} ${className}`}>
      <div className="flex items-center justify-between px-4 py-2.5 bg-base-900/70 border-b border-base-800">
        <div className="flex items-center gap-3">
          <span className="relative flex w-2 h-2">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-accent opacity-60"></span>
            <span className="relative inline-flex rounded-full w-2 h-2 bg-accent"></span>
          </span>
          <span className="text-[11px] text-base-400 tracking-wide">{title}</span>
        </div>
        <span className={`text-[10px] px-2 py-0.5 rounded-md border tracking-widest uppercase ${theme.chip}`}>{theme.label}</span>
      </div>

      <div className="relative">
        <pre aria-hidden className={`px-4 py-3 text-[11px] sm:text-xs leading-snug min-h-[220px] whitespace-pre tnum ${theme.text}`}>
          {frames[idx % frames.length]}
        </pre>
        <div className="absolute bottom-0 left-0 right-0 pointer-events-none h-8 bg-gradient-to-t from-base-ink to-transparent"></div>
      </div>

      <div className="border-t border-base-800 bg-base-950/70 px-4 py-2 space-y-0.5">
        {visibleLogs.map((l, i) => (
          <div key={`${l}-${i}`} className={`text-[11px] truncate ${i === visibleLogs.length - 1 ? 'text-base-300' : 'text-base-600'}`}>
            <span className="text-accent mr-2">&gt;</span>
            {l}
          </div>
        ))}
        <div className="flex items-center justify-between pt-1">
          <span className="text-[10px] uppercase tracking-widest text-base-500">{theme.step}</span>
