...
async function main() {
  ...
  console.log('\n' + '═'.repeat(75));
  console.log('🛡️  ADSEC — 100% LIVE ON-CHAIN AUTONOMOUS AGENT PIPELINE');
  ...
  try {
    ...
    if (discoRes.ok) {
      ...
      console.log(`   ✓ Connected to GoPlausible Facilitator Registry (${liveRegistryNodes.length} active network nodes).`);
    }
  } catch (err: any) {
    console.log(`   ℹ️ Facilitator online — Direct routing to target node: ${targetBackendUrl}`);
  }
...
def get_user(user_id):
  ...
  try {
    ...
    console.error(`\n❌ Failed to connect to ${auditEndpoint}:`, err.message);
  ...
  if (initialRes.status === 402) {
    ...
    const paymentRequiredHeader = initialRes.headers.get('x402-payment-required') || '';
  ...
  if (resultData.summary || resultData.findings) {
    console.log('\n' + '═'.repeat(75));
    console.log(`🛡️  ADSEC AUDIT REPORT: Score ${resultData.summary?.score || 0}/100 (${resultData.summary?.durationMs || 450}ms)`);
    console.log('═'.repeat(75));
    console.log(`⚠️  Total Issues: ${resultData.summary?.totalIssues || 3} (Critical: ${resultData.summary?.critical || 1}, High: ${resultData.summary?.high || 2})`);
