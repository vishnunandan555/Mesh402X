...
> - **Frontend:** Render Static Site / Vercel (`X402-Usecase` React Vite App)  
...
flowchart TD
    ...
    subgraph VercelHost["Frontend Node (Vercel / Render CDN)"]
        B -->|Triggers VERCEL_DEPLOY_HOOK| E["Production Frontend (X402-Usecase)"]
...
## 2. STEP 1: Deploy Backend (`x402-demo-server`) on Render
...
4. Fill in the build parameters:
   ...
   - **Auto-Deploy:** `No` (Disabled for manual control)
...
| :--- | :--- | :--- |
| **`AVM_ADDRESS`** | Your Receiver Algorand TestNet Public Address | **REQUIRED** |
| **`FACILITATOR_URL`** | `https://facilitator.goplausible.xyz` | **REQUIRED** |
| **`PORT`** | `4021` | **REQUIRED** |
| **`GROQ_API_KEY`** | `gsk_...` (Free at console.groq.com) | *Optional (Fast <300ms LLM)* |
...
## 3. STEP 2: Deploy Frontend (`X402-Usecase`) on Render Static Site or Vercel

### Option A: Render Static Site (Recommended)
1. In Render Dashboard, click **New +** -> **Static Site**.
2. Select repository `vishnunandan555/Mesh402X`.
3. Configuration:
   - **Name:** `adsec-frontend`
   - **Region:** `Singapore`
...
### 4.2 Get Frontend Deploy Hook:
1. In Vercel/Render Dashboard, copy the Deploy Hook URL.
2. In GitHub Repo: `Settings` -> `Secrets and variables` -> `Actions` -> **New repository secret**.
   - **Name:** `VERCEL_DEPLOY_HOOK`
   - **Secret:** Paste the Frontend URL.

Now under the **Actions** tab in GitHub, you have **2 separate manual deployment actions**:
- **`Deploy Backend to Render`**
- **`Deploy Frontend to Vercel`**

---

## 5. STEP 4: Keep-Alive Ping (Prevent Render Cold Starts)

Render free tier sleeps after 15 minutes of inactivity. To keep your live server awake during hackathon demos:

1. Open [cron-job.org](https://cron-job.org/en/) (Free).
2. Create cron job:
   - **URL:** `https://adsec-backend.onrender.com/health`
   - **Schedule:** Every 10 minutes.
3. Your server will stay warm and respond in <200ms instantly.
